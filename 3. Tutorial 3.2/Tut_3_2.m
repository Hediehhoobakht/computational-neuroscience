...
saveas(f1, sprintf('f1_ISIHistogram.png'));
CV=std(ISI)/mean(ISI)
...
No=No_spikes_100ms_window
mean_=(mean(No_spikes_100ms_window))
std_=((std(No_spikes_100ms_window))^2)
...
xlabel('Time(sec.)');
ylabel('Fano Factor');   
saveas(f2, sprintf('f2_FanoFactor.png'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%1.b
b=1e-9;
...
No_b=No_spikes_100ms_window_b
mean_b=(mean(No_spikes_100ms_window_b))
std_b=((std(No_spikes_100ms_window_b))^2)
...
xlabel('Time(sec.)');
ylabel('Fano Factor');   
saveas(f4, sprintf('f4_FanoFactor_b.png'));
...
xlabel('ISI(Sec.)');
ylabel('Count of ISIs');
saveas(f5, sprintf('f5_ISIHistogram_1c_0.png'));
...
xlabel('ISI(Sec.)');
ylabel('Count of ISIs');
saveas(f6, sprintf('f6_ISIHistogram_1c_01.png'));
...
xlabel('ISI(Sec.)');
ylabel('Count of ISIs');
saveas(f7, sprintf('f7_ISIHistogram_1c_02.png'));
...
saveas(f8, sprintf('f8_PoissonHistogram.png'));
CV=std(ISI)/mean(ISI)
...
No9=No_spikes_100ms_window
mean9=(mean(No_spikes_100ms_window))
std9=((std(No_spikes_100ms_window))^2)
fano = ((std(No_spikes_100ms_window))^2)/(mean(No_spikes_100ms_window))
...
xlabel('Time(sec.)');
ylabel('Fano Factor');   
saveas(f9, sprintf('f9_FanoFactor.png'));
%%2b-B
t=0:dt:10-dt;
for k=1:1000
...
grid on;
saveas(f10, sprintf('f10_FanoFactor.png'));
...
grid on;
saveas(f11, sprintf('f11_FanoFactor.png'));
...
grid on;
saveas(f12, sprintf('f12_FanoFactor_200ms.png'));
%%%%%
t=0:dt:10-dt;
...
title('Fano Factor as a Function of Elapsed Time 1000ms-inhomogenous');
grid on;
saveas(f13, sprintf('f13_FanoFactor_1000ms.png'));
N_T = cumsum(spikemat,2);
variance_across_rows = var(N_T,0, 1)  
mean_across_rows = mean(N_T, 1)   
fano_factor = variance_across_rows ./ mean_across_rows
...
title('Fano Factor as a Function of Elapsed Time');
grid on;
saveas(f14, sprintf('f14_FanoFactor-1000.png'));
window_size = 0.2; % 200 ms window size
trial_duration = 10; % Duration of each trial (in seconds), provide the actual value
% Calculate the number of time windows
num_trials =(trial_duration / window_size);
...
title('Fano Factor as a Function of Elapsed Time 200ms');
grid on;
saveas(f15, sprintf('f15_FanoFactor_200ms.png'));